...
for sb in range(len(subjects)):
	...
	sub_qval_diff_workflow_dir = os.path.join(workflow_dir,subjects[sb],"med_diff","feedback_qval_diff")
	sub_gf_rpe_workflow_dir = os.path.join(workflow_dir,subjects[sb],"med_diff","gf_RPE")
	...
	if not os.path.exists(sub_gf_rpe_workflow_dir):
		os.makedirs(sub_gf_rpe_workflow_dir)	
	...
	if subjects[sb] not in ('sub-203','sub-209','sub-213','sub-215'):
		...
		sub_on_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"on","train","across_runs","feedback_qval_diff")
		sub_on_gf_rpe_dir = os.path.join(workflow_dir,subjects[sb],"on","train","across_runs","gf_RPE")
		...
		sub_off_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"off","train","across_runs","feedback_qval_diff")
		sub_off_gf_rpe_dir = os.path.join(workflow_dir,subjects[sb],"off","train","across_runs","gf_RPE")
		...
		feat_gf_rpe_dirs = [sub_on_gf_rpe_dir,sub_off_gf_rpe_dir]
		...
		sub_on_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"on","train","run-1","feedback_qval_diff")	
		sub_on_gf_rpe_dir = os.path.join(workflow_dir,subjects[sb],"on","train","run-1","gf_RPE")
		...
		sub_off_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"off","train","run-1","feedback_qval_diff")
		sub_off_gf_rpe_dir = os.path.join(workflow_dir,subjects[sb],"off","train","run-1","gf_RPE")
		...
		cope_gf_rpe_dirs = [sub_on_gf_rpe_dir,sub_off_gf_rpe_dir]
		...
		sub_on_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"on","train","run-2","feedback_qval_diff")	
		sub_on_gf_rpe_dir = os.path.join(workflow_dir,subjects[sb],"on","train","run-2","gf_RPE")
		...
		sub_off_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"off","train","run-2","feedback_qval_diff")
		sub_off_gf_rpe_dir = os.path.join(workflow_dir,subjects[sb],"off","train","run-2","gf_RPE")
	...
	## 5. Goed/Fout & RPE workflow ##

	contrasts = [1,2,3,4,5] # Goed>Fout, Fout>Goed, Goed, Fout, RPE
	...
	for contrast in contrasts: 
		...
		if subjects[sb] not in ('sub-203','sub-209', 'sub-213','sub-215'):
			# collecting copes from feat directories (for those subs that had across_runs)
			copes = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','cope1.nii.gz') for x in feat_gf_rpe_dirs]
			varcopes = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','varcope1.nii.gz') for x in feat_gf_rpe_dirs]
			tstats = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','tstat1.nii.gz') for x in feat_gf_rpe_dirs]

		elif subjects[sb] in ('sub-203','sub-209', 'sub-213','sub-215'):
			# appending remaining copes for those subs that had only one run
			copes = [os.path.join(x,'workflow','FE','stats','%s%i.nii.gz'%('cope',contrast)) for x in cope_gf_rpe_dirs]
			varcopes = [os.path.join(x,'workflow','FE','stats','%s%i.nii.gz'%('varcope',contrast))for x in cope_gf_rpe_dirs]
			tstats = [os.path.join(x,'workflow','FE','stats','%s%i.nii.gz'%('tstat',contrast))for x in cope_gf_rpe_dirs]


		# Create workflow
		Parkflow_gf_rpe_across_sess = Workflow(name='workflow')
		Parkflow_gf_rpe_across_sess.base_dir = os.path.join(sub_gf_rpe_workflow_dir,"cope"+str(contrast))

		if not os.path.exists(Parkflow_gf_rpe_across_sess.base_dir):
			os.makedirs(Parkflow_gf_rpe_across_sess.base_dir)
		...
		FE=Node(interface=fsl.FLAMEO(
			...
			mask_file=maskfile),
			name='FE',
			stats_dir=os.path.join(Parkflow_gf_rpe_across_sess.base_dir,'stats'))

		
		Parkflow_gf_rpe_across_sess.connect([(copemerge,FE,[('merged_file','cope_file')]),
						(varcopemerge,FE,[('merged_file','var_cope_file')]),
						(multregmodel,FE,[('design_mat','design_file'),
										('design_con','t_con_file'),
										('design_grp','cov_split_file')]),
		...
		Parkflow_gf_rpe_across_sess.write_graph(graph2use='colored')
		Parkflow_gf_rpe_across_sess.run()
