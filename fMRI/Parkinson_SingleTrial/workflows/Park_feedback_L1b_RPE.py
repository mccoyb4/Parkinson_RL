...
for sb in range(len(subjects)):
	...
	for run_nr in runs:
		...
		if hc_or_pd == 'hc':
			...
			sub_qval_diff_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-"+run_nr,"feedback_qval_diff")		
		...
		multregmodel.inputs.regressors = dict(reg1=list(EV_qval_diff_design_df['feedback']),reg2=list(EV_qval_diff_design_df['qval_diff_demeaned']))
