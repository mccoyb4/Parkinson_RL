...
feat_rpe_dirs_all_subs, feat_arpe_dirs_all_subs, feat_qval_dirs_all_subs, feat_qval_diff_dirs_all_subs, feat_gf_rpe_dirs_all_subs = [[] for i in range(5)]

k_beta,k_ag,k_al, ledd, quip = [[] for i in range(5)]
...
for sb in range(len(subjects)):
	...
	sub_qval_diff_feat_dir = os.path.join(sub_workflow_dir,"feedback_qval_diff")
	sub_gf_rpe_feat_dir = os.path.join(sub_workflow_dir,"gf_RPE")
	...
	feat_gf_rpe_dirs_all_subs.append(sub_gf_rpe_feat_dir)
...
# LEDD and QUIP-RS scores
ledd_filename = os.path.join('/home/shared/2016/Parkinson/data','Bayesian_learning_parameters','LEDD.csv')
ledd_quip_scores = pd.read_csv(ledd_filename)
ledd_score = np.array(ledd_quip_scores['LEDD']/1000) # converted to grams instead of mg
quip_score = np.array(ledd_quip_scores['QUIP'])

ledd_demeaned = ledd_score - ledd_score.mean()
quip_demeaned = quip_score - quip_score.mean()
...
for contrast in contrasts: 
	...
	multregmodel.inputs.contrasts = [["cope"+str(contrast)+'_mean', 'T',['reg1'],[1]],['ledd', 'T',['reg2'],[1]], ['quip', 'T',['reg3'],[1]]] 
	multregmodel.inputs.regressors = dict(reg1=list(np.ones(len(copes_on_off))),reg2=list(ledd_demeaned), reg3=list(quip_demeaned))
	...
	# # # Cluster correction for ledd covariate
	...
	# clusterout = os.popen(clustercmd).read()
	# f1=open('thres_cluster_ledd_2.3_fwe_table.txt','w+')
	# f1.write(clusterout)
	# f1.close()

	# # # Cluster correction for quip covariate
	...
	# clusterout = os.popen(clustercmd).read()
	# f1=open('thres_cluster_quip_2.3_fwe_table.txt','w+')
	# f1.write(clusterout)
	# f1.close()
...
for contrast in contrasts: 
	...
	multregmodel.inputs.contrasts = [["cope"+str(contrast)+'_mean', 'T',['reg1'],[1]],['ledd', 'T',['reg2'],[1]], ['quip', 'T',['reg3'],[1]]] 
	multregmodel.inputs.regressors = dict(reg1=list(np.ones(len(copes_off_on))),reg2=list(ledd_demeaned), reg3=list(quip_demeaned))
...
# 	copes_on_off = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','cope1.nii.gz') for x in feat_qval_diff_dirs_all_subs]
# 	varcopes_on_off = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','varcope1.nii.gz') for x in feat_qval_diff_dirs_all_subs]
# 	tstats_on_off = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','tstat1.nii.gz') for x in feat_qval_diff_dirs_all_subs]

# 	# Create workflow
# 	Parkflow_group_qval_diff = Workflow(name='workflow')
# 	Parkflow_group_qval_diff.base_dir = os.path.join(group_dir,"on_off","feedback_qval_diff","cope"+str(contrast))
...
# 	smoothcmd = 'smoothest -r res4d -d %i -m mask'%(len(feat_qval_diff_dirs_all_subs)-1)
# 	smooth = os.popen(smoothcmd).read().split("\n")
# 	smoothn = [x.split(' ')[1] for x in smooth[:-1]]
...
# 	copes_off_on = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','cope2.nii.gz') for x in feat_qval_diff_dirs_all_subs]
# 	varcopes_off_on = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','varcope2.nii.gz') for x in feat_qval_diff_dirs_all_subs]
# 	tstats_off_on = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','tstat2.nii.gz') for x in feat_qval_diff_dirs_all_subs]

# 	# Create workflow
# 	Parkflow_group_qval_diff = Workflow(name='workflow')
# 	Parkflow_group_qval_diff.base_dir = os.path.join(group_dir,"off_on","feedback_qval_diff","cope"+str(contrast))
...
# 	copes_on_off = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','cope1.nii.gz') for x in feat_gf_rpe_dirs_all_subs]
# 	varcopes_on_off = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','varcope1.nii.gz') for x in feat_gf_rpe_dirs_all_subs]
# 	tstats_on_off = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','tstat1.nii.gz') for x in feat_gf_rpe_dirs_all_subs]

# 	# Create workflow
# 	Parkflow_group_gf_rpe = Workflow(name='workflow')
# 	Parkflow_group_gf_rpe.base_dir = os.path.join(group_dir,"on_off","gf_RPE","cope"+str(contrast))

# 	if not os.path.exists(Parkflow_group_gf_rpe.base_dir):
# 		os.makedirs(Parkflow_group_gf_rpe.base_dir)
...
# 		infer_outliers=True),
# 		name='flame12',
# 		stats_dir=os.path.join(Parkflow_group_gf_rpe.base_dir,'stats',))

# 	# Use level2model if not using covariate
# 	Parkflow_group_gf_rpe.connect([(copemerge,flame12,[('merged_file','cope_file')]),
# 					(varcopemerge,flame12,[('merged_file','var_cope_file')]),
# 					(level2model,flame12,[('design_mat','design_file'),
# 									('design_con','t_con_file'),
# 									('design_grp','cov_split_file')])
...
# 	Parkflow_group_gf_rpe.write_graph(graph2use='colored')
# 	Parkflow_group_gf_rpe.run()

# 	# Cluster correction
# 	os.chdir(os.path.join(Parkflow_group_gf_rpe.base_dir,'workflow','flame12','stats'))

# 	smoothcmd = 'smoothest -r res4d -d %i -m mask'%(len(feat_gf_rpe_dirs_all_subs)-1)
# 	smooth = os.popen(smoothcmd).read().split("\n")
# 	smoothn = [x.split(' ')[1] for x in smooth[:-1]]
...
# 	copes_off_on = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','cope2.nii.gz') for x in feat_gf_rpe_dirs_all_subs]
# 	varcopes_off_on = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','varcope2.nii.gz') for x in feat_gf_rpe_dirs_all_subs]
# 	tstats_off_on = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','tstat2.nii.gz') for x in feat_gf_rpe_dirs_all_subs]

# 	# Create workflow
# 	Parkflow_group_gf_rpe = Workflow(name='workflow')
# 	Parkflow_group_gf_rpe.base_dir = os.path.join(group_dir,"off_on","gf_RPE","cope"+str(contrast))
