...
feat_rpe_dirs_all_subs, feat_arpe_dirs_all_subs, feat_qval_dirs_all_subs, feat_qval_diff_dirs_all_subs = [[] for i in range(4)]
...
for sb in range(len(subjects)):
	...
	sub_rpe_feat_dir = os.path.join(sub_workflow_dir,"feedback_RPE_ortho")
	sub_arpe_feat_dir = os.path.join(sub_workflow_dir,"feedback_aRPE_ortho")
	sub_qval_feat_dir = os.path.join(sub_workflow_dir,"feedback_qval_ortho")
	sub_qval_diff_feat_dir = os.path.join(sub_workflow_dir,"feedback_qval_diff_ortho")
...
k_ag_al_diff = k_ag - k_al
k_ag_al_diff_demeaned = k_ag_al_diff - k_ag_al_diff[~np.isnan(k_ag_al_diff)].mean()
...
for contrast in contrasts: 
	...
	Parkflow_group_rpe.base_dir = os.path.join(group_dir,"on_off","feedback_RPE_ortho","cope"+str(contrast))
	...
	multregmodel.inputs.contrasts = [["cope"+str(contrast)+'_mean', 'T',['reg1'],[1]],['k_al', 'T',['reg2'],[1]]] 
	multregmodel.inputs.regressors = dict(reg1=list(np.ones(len(copes_on_off))),reg2=list(k_ag_al_diff_demeaned))
	...
	# clusterout = os.popen(clustercmd).read()
	# f1=open('thres_cluster_k_ag_al_diff_2.3_fwe_table.txt','w+')
	# f1.write(clusterout)
	# f1.close()
...
for contrast in contrasts: 
	...
	# Create workflow
	Parkflow_group_rpe = Workflow(name='workflow')
	Parkflow_group_rpe.base_dir = os.path.join(group_dir,"off_on","feedback_RPE_ortho","cope"+str(contrast))
	...
	# Parkflow_group_rpe.connect([(copemerge,flame122,[('merged_file','cope_file')]),
	# 		(varcopemerge,flame122,[('merged_file','var_cope_file')]),
	# 		(multregmodel,flame122,[('design_mat','design_file'),
	# 						('design_con','t_con_file'),
	# 						('design_grp','cov_split_file')])
	# 		,
	# 		])
...
for contrast in contrasts: 
	...
	Parkflow_group_arpe.base_dir = os.path.join(group_dir,"on_off","feedback_aRPE_ortho","cope"+str(contrast))
...
for contrast in contrasts: 
	...
	# Create workflow
	Parkflow_group_arpe = Workflow(name='workflow')
	Parkflow_group_arpe.base_dir = os.path.join(group_dir,"off_on","feedback_aRPE_ortho","cope"+str(contrast))
...
for contrast in contrasts: 
	...
	Parkflow_group_qval.base_dir = os.path.join(group_dir,"on_off","feedback_qval_ortho","cope"+str(contrast))
...
for contrast in contrasts: 
	...
	# Create workflow
	Parkflow_group_qval = Workflow(name='workflow')
	Parkflow_group_qval.base_dir = os.path.join(group_dir,"off_on","feedback_qval_ortho","cope"+str(contrast))
...
for contrast in contrasts: 
	...
	Parkflow_group_qval_diff.base_dir = os.path.join(group_dir,"on_off","feedback_qval_diff_ortho","cope"+str(contrast))
...
for contrast in contrasts: 
	...
	# Create workflow
	Parkflow_group_qval_diff = Workflow(name='workflow')
	Parkflow_group_qval_diff.base_dir = os.path.join(group_dir,"off_on","feedback_qval_diff_ortho","cope"+str(contrast))
