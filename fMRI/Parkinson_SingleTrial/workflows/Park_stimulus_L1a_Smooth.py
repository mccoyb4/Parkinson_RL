...
# Based on B McCoy single trial feedback analysis
# Outputs to single_trial_analysis/smooth_4 
...
for sb in range(len(subjects)):
	...
	if hc_or_pd == 'hc':
	    ...
	    sub_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","run-"+run_nr,"stimulus")
	    ...
	    sub_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-"+run_nr,"stimulus")
	...
	events.loc[events['Outcome']=='omission','RT'] = 3.00 # If there is an omission, change 'RT' from NaN to 3.0 seconds.
	...
	trials, stimulus_onset_per_trial, stimulus_duration_per_trial = [[] for i in range(3)]

	for i in range(len(events)):
	    trials.append("trial_"+str(i+1)+"_pair_"+str(events.Pair[i])+"_choice_"+str(events.Choice[i])+"_feedback_"+str(events.Outcome[i]))
	    stimulus_onset_per_trial.append([events.onset[i]])
	    stimulus_duration_per_trial.append([events.RT[i]])

	info = [Bunch(conditions=trials,
					onsets=stimulus_onset_per_trial,
					durations=stimulus_duration_per_trial,
	...
	smoothed = Node(Smooth(
		in_file = os.path.join(sub_workflow_dir, subjects[sb] + "_task-train_run-" + run_nr + "_bold_space-MNI152NLin2009cAsym_preproc_roi.nii.gz"),
	...
	Parkflow.connect([(smoothed, sgfilter, [('smoothed_file', 'in_file')]),
