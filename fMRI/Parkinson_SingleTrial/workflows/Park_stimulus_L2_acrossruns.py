...
for sb in range(len(subjects)):
	...
	if hc_or_pd == 'hc':
		...
		sub_qval_diff_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","across_runs","stimulus_QvalDiff")
		sub_choice_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","across_runs","choice_QvalDiff")

		# Qval_diff dirs
		sub_run1_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"train","run-1","stimulus_QvalDiff","workflow",'FE')
		sub_run2_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],"train","run-2","stimulus_QvalDiff","workflow",'FE')

		# Choice dirs
		sub_run1_choice_dir = os.path.join(workflow_dir,subjects[sb],"train","run-1","choice_QvalDiff","workflow",'FE')
		sub_run2_choice_dir = os.path.join(workflow_dir,subjects[sb],"train","run-2","choice_QvalDiff","workflow",'FE')


	elif hc_or_pd == 'pd':

		sub_qval_diff_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","across_runs","stimulus_QvalDiff")
		sub_choice_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","across_runs","choice_QvalDiff")

		# Qval_diff dirs
		sub_run1_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-1","stimulus_QvalDiff","workflow",'FE')
		sub_run2_qval_diff_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-2","stimulus_QvalDiff","workflow",'FE')

		# Choice dirs
		sub_run1_choice_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-1","choice_QvalDiff","workflow",'FE')
		sub_run2_choice_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-2","choice_QvalDiff","workflow",'FE')
	...
	feat_choice_dirs = [sub_run1_choice_dir,sub_run2_choice_dir]

	## ACROSS-RUN MASK ##
	maskfile = os.path.join(workflow_dir,"masks","mni2func_mask_dil.nii.gz")


	## 1. Qval_diff (chosen - unchosen value) workflow ##

	contrasts = [1,2,3] # Stimulus, Qval_diff+, Qval_diff-
	...
	## 1. Choice (correct/incorrect) workflow ##

	# contrasts = [1,2,3,4,5,6] # Correct, Incorrect, Qval_diff+, Qval_diff-, Correct>Incorrect, Incorrect>Correct

	# for contrast in contrasts: 

	# 	copes = [os.path.join(x,'stats','%s%i.nii.gz'%('cope',contrast)) for x in feat_choice_dirs]
	# 	varcopes = [os.path.join(x,'stats','%s%i.nii.gz'%('varcope',contrast)) for x in feat_choice_dirs]
	# 	tstats = [os.path.join(x,'stats','%s%i.nii.gz'%('tstat',contrast)) for x in feat_choice_dirs]

	# 	# Create workflow
	# 	Parkflow_choice_across_runs = Workflow(name='workflow')
	# 	Parkflow_choice_across_runs.base_dir = os.path.join(sub_choice_workflow_dir,"cope"+str(contrast))

	# 	if not os.path.exists(Parkflow_choice_across_runs.base_dir):
	# 		os.makedirs(Parkflow_choice_across_runs.base_dir)
	...
	# 		mask_file=maskfile),
	# 		name='FE',
	# 		stats_dir=os.path.join(Parkflow_choice_across_runs.base_dir,'stats'))

		
	# 	Parkflow_choice_across_runs.connect([(copemerge,FE,[('merged_file','cope_file')]),
	# 					(varcopemerge,FE,[('merged_file','var_cope_file')]),
	# 					(level2model,FE,[('design_mat','design_file'),
	# 									('design_con','t_con_file'),
	# 									('design_grp','cov_split_file')]),
	...
	# 	Parkflow_choice_across_runs.write_graph(graph2use='colored')
	# 	Parkflow_choice_across_runs.run()
