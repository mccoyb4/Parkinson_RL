...
for sb in range(len(subjects)):
	...
	for run_nr in runs:
		...
		if analysis_type == 'single-trial':
			...
			# sub-215 on run-2 only has 8 Cosines from fmriprep output (rather than 11), as scan was shorter than others 
			# (participant needed to get out)

			if ((sub_id == 'sub-215') and (pd_on_off=='on') and (run_nr=="2")):
				check_sub_215=1
			else:
				check_sub_215=0
		...
		Parkflow.connect([(masker, smoothed, [('out_file', 'in_file')]),
				 ...
				 (l1featmodel, filmgls, [('design_file','design_file')])
				 ])

		Parkflow.write_graph(graph2use='colored')
		Parkflow.run()

		# Adjusted to run on Lisa : l1model.design_file is a unicode string there, need it to be standard string.
		# l1featmodel.outputs.design_file = os.path.join(sub_workflow_dir,'workflow','l1model','run0.mat')

		# Parkflow2 = Workflow(name='workflow')
		# Parkflow2.base_dir = sub_workflow_dir
		# Parkflow2.connect([(sgfilter, filmgls, [('sg_file', 'in_file')]),
		# 		 (l1featmodel, filmgls, [('design_file','design_file')])
		# 		 ])

		# Parkflow2.write_graph(graph2use='colored')
		# Parkflow2.run()
