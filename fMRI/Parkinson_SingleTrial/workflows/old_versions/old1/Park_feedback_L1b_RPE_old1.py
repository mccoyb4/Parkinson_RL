...
smooth = str(sys.argv[5]) # use smoothed output or not? 0 = no smooth, 1 = smooth
ae_or_lisa = str(sys.argv[6]) # 'ae' or 'lisa' server
...
for sb in range(len(subjects)):
	...
	for run_nr in runs:
		...
		if hc_or_pd == 'hc':
			...
			sub_gf_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-"+run_nr,"gf_RPE")
		...
		###### NIPYPE WORKFLOW 4: Separate Goed/Fout EVs with RPE as parametric modulator ######
