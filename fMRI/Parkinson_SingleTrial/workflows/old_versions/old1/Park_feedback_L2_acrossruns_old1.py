...
for sb in range(len(subjects)):
	...
	if hc_or_pd == 'hc':
		...
		sub_rpe_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","across_runs","feedback_RPE")
		sub_arpe_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","across_runs","feedback_aRPE")
		sub_qval_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","across_runs","feedback_qval")
		sub_gf_workflow_dir = os.path.join(workflow_dir,subjects[sb],"train","across_runs","gf_RPE")

		# RPE dirs
		sub_run1_rpe_dir = os.path.join(workflow_dir,subjects[sb],"train","run-1","feedback_RPE","workflow",'FE')
		sub_run2_rpe_dir = os.path.join(workflow_dir,subjects[sb],"train","run-2","feedback_RPE","workflow",'FE')

		# alpha RPE dirs
		sub_run1_arpe_dir = os.path.join(workflow_dir,subjects[sb],"train","run-1","feedback_aRPE","workflow",'FE')
		sub_run2_arpe_dir = os.path.join(workflow_dir,subjects[sb],"train","run-2","feedback_aRPE","workflow",'FE')		

		# Qval update dirs
		sub_run1_qval_dir = os.path.join(workflow_dir,subjects[sb],"train","run-1","feedback_qval","workflow",'FE')
		sub_run2_qval_dir = os.path.join(workflow_dir,subjects[sb],"train","run-2","feedback_qval","workflow",'FE')

		# Goed/Fout RPE dirs
		sub_run1_gf_dir = os.path.join(workflow_dir,subjects[sb],"train","run-1","gf_RPE","workflow",'FE')
		sub_run2_gf_dir = os.path.join(workflow_dir,subjects[sb],"train","run-2","gf_RPE","workflow",'FE')

	elif hc_or_pd == 'pd':

		sub_rpe_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","across_runs","feedback_RPE")
		sub_arpe_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","across_runs","feedback_aRPE")
		sub_qval_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","across_runs","feedback_qval")
		sub_gf_workflow_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","across_runs","gf_RPE")

		# RPE dirs
		sub_run1_rpe_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-1","feedback_RPE","workflow",'FE')
		sub_run2_rpe_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-2","feedback_RPE","workflow",'FE')
		
		# alpha RPE dirs
		sub_run1_arpe_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-1","feedback_aRPE","workflow",'FE')
		sub_run2_arpe_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-2","feedback_aRPE","workflow",'FE')

		# Qval update dirs
		sub_run1_qval_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-1","feedback_qval","workflow",'FE')
		sub_run2_qval_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-2","feedback_qval","workflow",'FE')

		# Goed/Fout RPE dirs
		sub_run1_gf_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-1","gf_RPE","workflow",'FE')
		sub_run2_gf_dir = os.path.join(workflow_dir,subjects[sb],pd_on_off,"train","run-2","gf_RPE","workflow",'FE')
	...
	## CREATE ACROSS-RUN MASK ##

	runmaskfile_rpe = os.path.join(sub_rpe_workflow_dir,"mask.nii.gz") 
	runmaskfile_arpe = os.path.join(sub_arpe_workflow_dir,"mask.nii.gz") 
	runmaskfile_qval = os.path.join(sub_qval_workflow_dir,"mask.nii.gz")
	runmaskfile_gf = os.path.join(sub_gf_workflow_dir,"mask.nii.gz")  

	mask = np.zeros([60,71,60,len(feat_rpe_dirs)])
	k=0
	for this_run in ["1","2"]:
		maskfile = os.path.join(fmriprep_dir,"fmriprep",subjects[sb],'func',subjects[sb]+"_task-train_run-"+this_run+"_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz")
	...
	maskimg.to_filename(runmaskfile_rpe)
	maskimg.to_filename(runmaskfile_arpe)
	maskimg.to_filename(runmaskfile_qval)
	maskimg.to_filename(runmaskfile_gf)
