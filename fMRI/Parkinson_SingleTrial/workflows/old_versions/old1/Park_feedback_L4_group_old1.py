...
if hc_or_pd == 'hc':
	...
	if ae_or_lisa == 'ae':
		...
		if smooth == '0':
			group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/hc/group"
			workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/hc"
		elif smooth == '1':
			group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/smooth/hc/group"
			workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis/smooth/hc"
		...
		if smooth == '0':
			group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/hc/group"
			workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/hc"
		elif smooth == '1':
			group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/smooth/hc/group"
			workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/smooth/hc"
	
elif hc_or_pd == 'pd':
	# 'sub-212' removed due to substantial signal loss of Nuc. Accumbens and Putamen
	subjects = ['sub-201','sub-202','sub-203','sub-204','sub-205','sub-206','sub-207','sub-208','sub-209','sub-210','sub-211','sub-213','sub-214','sub-215','sub-216','sub-217','sub-219','sub-220','sub-221','sub-222','sub-223','sub-224']
	...
	if pd_on_off == 'on':
		...
		if ae_or_lisa == 'ae':
			...
			if smooth == '0':
				group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/pd/group/on"
				workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/pd"
			elif smooth == '1':
				group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/smooth/pd/group/on"
			...
			if smooth == '0':
				group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/pd/group/on"
				workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/pd"
			elif smooth == '1':
				group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/smooth/pd/group/on"
		...
		if ae_or_lisa == 'ae':
			...
			if smooth == '0':
				group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/pd/group/off"
				workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/pd"
			elif smooth == '1':
				group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/smooth/pd/group/off"
				workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis/smooth/pd"
			...
			if smooth == '0':
				group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/pd/group/off"
				workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/pd"
			elif smooth == '1':
				group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/smooth/pd/group/off"
				workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/smooth/pd"
...
feat_rpe_dirs_all_subs, feat_arpe_dirs_all_subs, feat_qval_dirs_all_subs, feat_gf_dirs_all_subs = [[] for i in range(4)]
cope_rpe_dirs_all_subs, cope_arpe_dirs_all_subs, cope_qval_dirs_all_subs, cope_gf_dirs_all_subs = [[] for i in range(4)]
...
for sb in range(len(subjects)):
	...
	if subjects[sb] not in ('sub-203','sub-209', 'sub-213', 'sub-215', 'sub-115'):
		...
		sub_gf_feat_dir = os.path.join(sub_workflow_dir,"train","across_runs","gf_RPE")
		...
		feat_gf_dirs_all_subs.append(sub_gf_feat_dir)
		...
		sub_gf_cope_dir = os.path.join(sub_workflow_dir,"train","run-1","gf_RPE")
		...
		cope_gf_dirs_all_subs.append(sub_gf_cope_dir)
		...
		sub_gf_cope_dir = os.path.join(sub_workflow_dir,"train","run-2","gf_RPE")
...
mask = np.zeros([60,71,60,len(feat_rpe_dirs_all_subs) + len(cope_rpe_dirs_all_subs)]) # mask is same for rpe and arpe files
k=0

for sb in subjects:

	if sb not in ('sub-203','sub-209', 'sub-213','sub-215','sub-115'):
		maskfile = os.path.join(sub_rpe_feat_dir,"mask.nii.gz")
	elif sb in ('sub-203','sub-209','sub-215'):
		maskfile = os.path.join(fmriprep_dir,"fmriprep", sb, "func", sb + "_task-train_run-1_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz")
	elif sb in ('sub-213','sub-115'):
		maskfile = os.path.join(fmriprep_dir,"fmriprep", sb, "func", sb + "_task-train_run-2_bold_space-MNI152NLin2009cAsym_brainmask.nii.gz")

	if os.path.exists(maskfile):
		masksub = nib.load(maskfile)
		data = masksub.get_data()
		#print sb, data.shape, masksub.affine
		mask[:,:,:,k] = data
		k += 1
...
for contrast in contrasts: 
	...
	# collecting copes from feat directories (for those subs that had across_runs)
	copes = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','cope1.nii.gz') for x in feat_gf_dirs_all_subs]
	varcopes = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','varcope1.nii.gz') for x in feat_gf_dirs_all_subs]
	tstats = [os.path.join(x,'%s%i'%('cope',contrast),'workflow','FE','stats','tstat1.nii.gz') for x in feat_gf_dirs_all_subs]

	# appending remaining copes for those subs that had only one run
	for i in range(len(cope_rpe_dirs_all_subs)):
		copes.append(os.path.join(cope_gf_dirs_all_subs[i],'workflow','FE','stats','%s%i.nii.gz'%('cope',contrast)))
		varcopes.append(os.path.join(cope_gf_dirs_all_subs[i],'workflow','FE','stats','%s%i.nii.gz'%('varcope',contrast)))
		tstats.append(os.path.join(cope_gf_dirs_all_subs[i],'workflow','FE','stats','%s%i.nii.gz'%('tstat',contrast)))

	# Create workflow
	Parkflow_group_gf = Workflow(name='workflow')
	Parkflow_group_gf.base_dir = os.path.join(group_dir,"gf_RPE","cope"+str(contrast))

	if not os.path.exists(Parkflow_group_gf.base_dir):
		os.makedirs(Parkflow_group_gf.base_dir)
	...
	flame1=Node(interface=fsl.FLAMEO(
		...
		#infer_outliers=True),
		name='flame1',
		stats_dir=os.path.join(Parkflow_group_gf.base_dir,'stats'))

	
	Parkflow_group_gf.connect([(copemerge,flame1,[('merged_file','cope_file')]),
	...
	Parkflow_group_gf.write_graph(graph2use='colored')
	Parkflow_group_gf.run()

	# Cluster correction
	os.chdir(os.path.join(Parkflow_group_gf.base_dir,'workflow','flame1','stats'))

	smoothcmd = 'smoothest -r res4d -d %i -m mask'%(len(feat_gf_dirs_all_subs) + len(cope_gf_dirs_all_subs)-1)
	smooth = os.popen(smoothcmd).read().split("\n")
	smoothn = [x.split(' ')[1] for x in smooth[:-1]]
