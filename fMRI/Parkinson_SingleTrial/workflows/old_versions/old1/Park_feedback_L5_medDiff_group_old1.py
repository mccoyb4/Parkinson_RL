...
if ae_or_lisa == 'ae':
	...
	if smooth == '0':
		group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/pd/group/med_diff"
		workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis/no_smooth/pd"
	elif smooth == '1':
		group_dir = "/home/shared/2016/Parkinson/single_trial_analysis/smooth/pd/group/med_diff"
	...
	if smooth == '0':
		group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/pd/group/med_diff"
		workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/no_smooth/pd"
	elif smooth == '1':
		group_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis/smooth/pd/group/med_diff"
...
k_diff_demeaned = k_diff - k_diff[~np.isnan(k_diff)].mean()
...
for contrast in contrasts: 
	...
	multregmodel.inputs.contrasts = [["cope"+str(contrast)+'_mean', 'T',['reg1'],[1]],['k_diff', 'T',['reg2'],[1]]] 
	multregmodel.inputs.regressors = dict(reg1=list(np.ones(len(copes_on_off))),reg2=list(k_diff_demeaned))
	...
	# # Cluster correction for k_diff covariate
	# clustercmd = 'cluster -i zstat2 -c cope2 -t 2.3 -p 0.01 -d %s --volume=%s --othresh=thresh_cluster_2.3_fwe_zstat2 --connectivity=26 --mm'%(smoothn[0],smoothn[1])
	# clusterout = os.popen(clustercmd).read()
	# f1=open('thres_cluster_k_diff_2.3_fwe_table.txt','w+')
	# f1.write(clusterout)
	# f1.close()
...
for contrast in contrasts: 
	...
	multregmodel.inputs.contrasts = [["cope"+str(contrast)+'_mean', 'T',['reg1'],[1]],['k_diff', 'T',['reg2'],[1]]] 
	multregmodel.inputs.regressors = dict(reg1=list(np.ones(len(copes_off_on))),reg2=list(k_diff_demeaned))
