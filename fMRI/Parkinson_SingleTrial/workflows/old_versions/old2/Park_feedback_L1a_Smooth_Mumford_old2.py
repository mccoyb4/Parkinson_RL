...
analysis_type = str(sys.argv[4]) #'single-trial'
...
if hc_or_pd == 'hc':
	...
	if ae_or_lisa == 'ae':
		...
		workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis_Mumford/smooth/hc"

	elif ae_or_lisa == 'lisa':

		datadir='/nfs/bromccoy/Parkinson/data/hc'
		fmriprep_dir = '/nfs/bromccoy/Parkinson/fmriprep_preproc/hc_syn-sdc'
		workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis_Mumford/smooth/hc"
	...
	if ae_or_lisa=='ae':
		...
		workflow_dir = "/home/shared/2016/Parkinson/single_trial_analysis_Mumford/smooth/pd"
		...
		workflow_dir = "/nfs/bromccoy/Parkinson/single_trial_analysis_Mumford/smooth/pd"
...
for sb in range(len(subjects)):
	...
	for run_nr in runs:
		...
		# Remove first two TRs from nii, smooth, and mask the nii file
		# Do these already so that it saves once per run and not for each trial in the trial loop (it's ~460MB for these niftis)
		...
		smoothed = mem.cache(Smooth)
		smoothed(in_file = os.path.join(sub_workflow_dir, subjects[sb] + "_task-train_run-" + run_nr + "_bold_space-MNI152NLin2009cAsym_preproc_roi.nii.gz"),
		output_type = 'NIFTI_GZ',
		fwhm = analysis_info['fwhm_size'])

		masker = mem.cache(maths.ApplyMask)
		masker(in_file=os.path.join(sub_workflow_dir, subjects[sb] + "_task-train_run-" + run_nr +
		                 "_bold_space-MNI152NLin2009cAsym_preproc_roi.nii.gz"),
	    ...
	    # EV1 : Regressor (trial) of interest
		...
		# EV2 : All other trials
		other_trials, other_feedback_onset_per_trial,other_feedback_duration_per_trial = [[] for i in range(3)]

		for i in range(len(events)):
			other_trials.append(copy.deepcopy(trials))
			other_feedback_onset_per_trial.append(copy.deepcopy(feedback_onset_per_trial))
			other_feedback_duration_per_trial.append(copy.deepcopy(feedback_duration_per_trial))

		# EV2 : Remove trial of interest, so only other trials are left
		for i in range(len(events)):
			del other_trials[i][i]
			del other_feedback_onset_per_trial[i][i]
			del other_feedback_duration_per_trial[i][i]

		for tr in range(len(events)):

			GLM_EV1 = [[feedback_onset_per_trial[tr]],[feedback_duration_per_trial[tr]]]
			GLM_EV2 = [other_feedback_onset_per_trial[tr],other_feedback_duration_per_trial[tr]]

			info = [Bunch(conditions=[trials[tr],'other'],
			    onsets=[GLM_EV1[0],GLM_EV2[0]],
			    durations=[GLM_EV1[1],GLM_EV2[1]],
			    regressors=[list(confounds.FramewiseDisplacement.fillna(0)[analysis_info['remove_TRs']:]),
			            ...
			            list(confounds.Cosine00[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine01[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine02[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine03[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine04[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine05[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine06[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine07[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine08[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine09[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine10[analysis_info['remove_TRs']:]),
			            list(confounds.Cosine11[analysis_info['remove_TRs']:]), 
			            ],
			    regressor_names=['FramewiseDisplacement',
			            ...
			            'aCompCor04',
			            'aCompCor05',
			            'Cosine00',  ## Include Cosines as confounds (high-pass filtering)
			            'Cosine01',
			            'Cosine02',
			            'Cosine03',
			            'Cosine04',
			...
			# Get single trial contrast (t-stat)
			contrasts=[]
			contrasts.append(['trial_'+str(tr+1),'T', [trials[tr]],[1.0]])

			# Create workflow output (stats) dir for this particular trial
			sub_trial_workflow_dir = os.path.join(sub_workflow_dir,trials[tr])
			
			if not os.path.exists(sub_trial_workflow_dir):
				os.makedirs(sub_trial_workflow_dir)
			...
			sgfilter.inputs.in_file=os.path.join(sub_workflow_dir, subjects[sb] + "_task-train_run-1" + "_bold_space-MNI152NLin2009cAsym_preproc_roi_masked.nii.gz")
			...
			results_dir=os.path.join(sub_trial_workflow_dir,'stats')      
			), name='filmgls')

			Parkflow = Workflow(name='workflow')
			Parkflow.base_dir = sub_trial_workflow_dir

			Parkflow.connect([(sgfilter, l1, [('sg_file', 'functional_runs')]),
